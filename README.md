...
Description: Build an image recognition pipeline in AWS, using two EC2 instances, S3, SQS,
and Rekognition. The assignment must be done in Java on Amazon Linux VMs.
Create 2 EC2 instances (EC2 A and B), with Amazon Linux AMI, that will work in parallel.
Each instance will run a Java application. Instance A will read 10 images from an S3 bucket that
...
its index. Application must work no matter which instance starts first.
